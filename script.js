...
checkoutBtn.addEventListener("click", function () {
    // Simulação de autenticação (deve ser feita no backend com sessões)
    ...
    // Lógica para envio do pedido via WhatsApp
...
function selectPackage(name, pride, period) {
    // Atualiza o nome para incluir o período
    const fullName = `${name} - ${period}`;
    
    // Chama a função `addToCart` para adicionar o pacote ao carrinho
    addToCart(fullName, pride);
    
    // Mostra um feedback para o usuário
    Toastify({
        text: `${fullName} adicionado ao carrinho!`,
        duration: 3000,
        close: true,
        gravity: "top",
        position: "right",
